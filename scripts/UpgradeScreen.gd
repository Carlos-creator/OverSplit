...
func _make_card(upgrade: Dictionary) -> Control:
	...
	cat_lbl.text = "[" + upgrade.get("category", "") + "]"
	...
	desc_lbl.text = upgrade.get("desc_short", "")
	...
	long_lbl.text = upgrade.get("desc_long", "")
	...
	detail_btn.pressed.connect(func():
		...
		detail_btn.text = "Ocultar detalle" if long_lbl.visible else "Ver detalle"
	)
